...
//
	...
	extension Reactive where Base: UITextView {
		...
		public var didBeginEditing: ControlEvent<Void> {
			return ControlEvent<Void>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidBeginEditing(_:)))
				.map { _ in
					return ()
				})
		}
		...
		public var didEndEditing: ControlEvent<Void> {
			return ControlEvent<Void>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidEndEditing(_:)))
				.map { _ in
					return ()
				})
		}
		...
		public var didChange: ControlEvent<Void> {
			return ControlEvent<Void>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChange(_:)))
				.map { _ in
					return ()
				})
		}
		...
		public var didChangeSelection: ControlEvent<Void> {
			return ControlEvent<Void>(events: self.delegate.methodInvoked(#selector(UITextViewDelegate.textViewDidChangeSelection(_:)))
				.map { _ in
					return ()
				})
		}
